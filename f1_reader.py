...
# and then clean up data in the data frame.
# race_title and race_notes are added as metadata (attrs) to the data frame.
def read_f1_race_from_html_file_to_data_frame(file_path):
    ...
    # Delete last row if it just contains a string with notes, and instead add the notes as metadata
    last = df["POS."].iloc[-1]
    if isinstance(last, str) and len(last)>=4:
        df.attrs["race_notes"] = last
        df = df.iloc[:-1]
...
# Read the results of all F1 races of a season from html files to a dictionary
